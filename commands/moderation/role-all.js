...
module.exports = {
  ...
  run: async ({ interaction }) => {
    ...
    const embed = new EmbedBuilder()
      .setDescription(
        `<:xn_tick:1209742516832706642> **Stated Adding ${role} to ${interaction.guild.memberCount} members** `
      )
      .setColor("White");
    ...
    try {
      interaction.guild.members.cache.forEach(async (member) => {
        ...
        await embed.setDescription(`**Added ${role} to ${interaction.guild.memberCount} members**`)
        reply.edit({
            embed: [embed]
        })
      });
